...
export class DailyEntryModalComponent implements OnInit {
  ...
  loadExistingLogs(): void {
    ...
    // Load existing logs for the selected date
      ...
      next: (logs) => {
        ...
        this.goalsWithTasks.forEach(goalData => {
          goalData.tasks.forEach(task => {
            const existingLog = logs.find(log => log.taskId === task.id);
            this.taskLogs.set(task.id, {
              task: task,
              value: existingLog ? existingLog.value : false,
              notes: existingLog ? existingLog.notes || '' : '',
              logId: existingLog ? existingLog.id : undefined
            });
          });
        });
        this.loading = false;
  ...
  saveLogs(): void {
    ...
    this.taskLogs.forEach((taskLog, taskId) => {
      const dailyLog: DailyLog = {
        id: taskLog.logId || this.generateId(),
        userId: this.currentUserId,
        taskId: taskId,
        goalId: this.getGoalIdForTask(taskId),
      ...
      if (taskLog.logId) {
        // Update existing log
        saveOperations.push(this.dailyLogService.updateLog(dailyLog));
      } else {
        // Create new log
        saveOperations.push(this.dailyLogService.createLog(dailyLog));
      }
    });
